...
G  Given two sorted lists, L1 and L2 , write a procedure to compute L1 ∩ L2 using only
   the basic list operations. 

*/

// The L1 ∩ L2 set can be represented as a std::vector.
...
auto find_intersertion(const AList& L1, const BList& L2){
	...
	while( itL1 != L1.end() and itL2 != L2.end() ){
		if( *itL1 == *itL2 ){
			...
			intersection.push_back( common_val );
...
int main() {
	...
	std::vector<int> inc { find_intersertion(L1,L2)};
